...
export function parseThinkAndAnswer(text: string = '') {
  ...
  const beforeThink = normalizedText.slice(0, thinkStart);
  const afterThinkStart = normalizedText.slice(thinkStart + '<think>'.length);
  const thinkEnd = afterThinkStart.lastIndexOf('</think>');

  if (thinkEnd < 0) {
    return {
      thinking: afterThinkStart.replace(/<\/?think>/g, ''),
      answer: beforeThink,
      hasThinking: true,
      thinkingComplete: false,
    };
  ...
  const thinking = afterThinkStart.slice(0, thinkEnd).replace(/<\/?think>/g, '');
  const answer = `${beforeThink}${afterThinkStart.slice(thinkEnd + '</think>'.length)}`.replace(/<\/?think>/g, '');
...
export function getThinkingPreview(text: string = '', maxLines = 2) {
  return text
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean)
