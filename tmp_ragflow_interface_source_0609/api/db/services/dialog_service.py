...
def _default_answer_token_budget(max_tokens: int) -> int:
    try:
        configured = int(os.environ.get("RAGFLOW_CHAT_OUTPUT_TOKENS", "0"))
    except ValueError:
        configured = 0
    if configured > 0:
        return max(1, min(configured, max_tokens))

    try:
        reserve_ratio = float(os.environ.get("RAGFLOW_CHAT_OUTPUT_RESERVE_RATIO", "0.15"))
    except ValueError:
        reserve_ratio = 0.15
    reserve_ratio = min(max(reserve_ratio, 0.05), 0.5)
    return max(512, min(4096, int(max_tokens * reserve_ratio), max_tokens))


def _prepare_generation_config(dialog_llm_setting: dict | None, max_tokens: int, used_token_count: int) -> dict:
    gen_conf = deepcopy(dialog_llm_setting or {})
    answer_budget = _default_answer_token_budget(max_tokens)
    remaining_budget = max(1, max_tokens - used_token_count)
    if "max_tokens" in gen_conf and gen_conf["max_tokens"]:
        try:
            gen_conf["max_tokens"] = min(int(gen_conf["max_tokens"]), remaining_budget)
        except (TypeError, ValueError):
            gen_conf["max_tokens"] = min(answer_budget, remaining_budget)
    else:
        gen_conf["max_tokens"] = min(answer_budget, remaining_budget)
    return gen_conf
...
async def async_chat(dialog, messages, stream=True, **kwargs):
    ...
    kwargs["knowledge"] = "\n------\n" + "\n\n------\n\n".join(knowledges)
    answer_token_budget = _default_answer_token_budget(max_tokens)
    ...
    used_token_count, msg = message_fit_in(msg, max(1, max_tokens - answer_token_budget))
    if llm_type == "chat" and image_attachments:
        convert_last_user_msg_to_multimodal(msg, image_attachments, factory)
    assert len(msg) >= 2, f"message_fit_in has bug: {msg}"
    prompt = msg[0]["content"]

    gen_conf = _prepare_generation_config(dialog.llm_setting, max_tokens, used_token_count)
