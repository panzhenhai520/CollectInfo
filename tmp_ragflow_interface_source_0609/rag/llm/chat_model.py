...
class LiteLLMBase(ABC):
    ...
    def _is_ollama_native_chat(self) -> bool:
        return self.provider == SupportedLiteLLMProvider.Ollama

    def _ollama_model_name(self) -> str:
        prefix = LITELLM_PROVIDER_PREFIX.get(SupportedLiteLLMProvider.Ollama, "")
        if prefix and self.model_name.startswith(prefix):
            return self.model_name[len(prefix):]
        return self.model_name

    def _ollama_options(self, gen_conf: dict) -> dict:
        options = {}
        if gen_conf.get("max_tokens"):
            options["num_predict"] = int(gen_conf["max_tokens"])
        for src, dst in {
            ...
            if src in gen_conf and gen_conf[src] is not None:
                options[dst] = gen_conf[src]
        return options

    def _ollama_think(self, gen_conf: dict):
        if "think" in gen_conf:
            return gen_conf["think"]
        if "reasoning" in gen_conf:
            return bool(gen_conf["reasoning"])
        return os.environ.get("RAGFLOW_OLLAMA_THINK", "false").lower() in {"1", "true", "yes", "on"}

    async def _ollama_async_client(self):
        from ollama import AsyncClient

        headers = {}
        if self.api_key and self.api_key != "x":
            headers["Authorization"] = f"Bearer {self.api_key}"
        return AsyncClient(host=self.base_url, headers=headers or None)

    async def _ollama_async_chat(self, history, gen_conf):
        client = await self._ollama_async_client()
        options = self._ollama_options(gen_conf)
        response = await client.chat(
            model=self._ollama_model_name(),
            messages=history,
            stream=False,
            think=self._ollama_think(gen_conf),
            options=options or None,
        )
        content = response["message"]["content"] if isinstance(response, dict) else response.message.content
        prompt_tokens = response.get("prompt_eval_count", 0) if isinstance(response, dict) else getattr(response, "prompt_eval_count", 0)
        eval_tokens = response.get("eval_count", 0) if isinstance(response, dict) else getattr(response, "eval_count", 0)
        done_reason = response.get("done_reason", "") if isinstance(response, dict) else getattr(response, "done_reason", "")
        if done_reason == "length":
            content = self._length_stop(content or "")
        return (content or "").strip(), int(prompt_tokens or 0) + int(eval_tokens or 0)

    async def _ollama_async_chat_streamly(self, history, gen_conf):
        client = await self._ollama_async_client()
        options = self._ollama_options(gen_conf)
        ...
        async for chunk in stream:
            ...
            if done:
                prompt_tokens = chunk.get("prompt_eval_count", 0) if isinstance(chunk, dict) else getattr(chunk, "prompt_eval_count", 0)
                eval_tokens = chunk.get("eval_count", 0) if isinstance(chunk, dict) else getattr(chunk, "eval_count", 0)
                total_tokens = int(prompt_tokens or 0) + int(eval_tokens or 0)
                done_reason = chunk.get("done_reason", "") if isinstance(chunk, dict) else getattr(chunk, "done_reason", "")
                if done_reason == "length":
                    yield LENGTH_NOTIFICATION_CN
        yield total_tokens
    ...
    async def async_chat(self, system, history, gen_conf, **kwargs):
        ...
        if self._is_ollama_native_chat():
            return await self._ollama_async_chat(hist, {**gen_conf, **kwargs})
    ...
    async def async_chat_streamly(self, system, history, gen_conf, **kwargs):
        ...
        gen_conf = self._clean_conf(gen_conf)
        if self._is_ollama_native_chat():
            async for chunk in self._ollama_async_chat_streamly(history, {**gen_conf, **kwargs}):
                yield chunk
            return
        reasoning_start = False
        total_tokens = 0
