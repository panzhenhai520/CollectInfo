...
set LOG=%BASE%\auth-diagnose3-246.log
cd /d "%BASE%"
echo === %date% %time% === > "%LOG%"
echo === find auth files === >> "%LOG%"
docker exec ragflow-v0256-upgrade-ragflow-cpu-1 sh -lc "find /ragflow/api/apps -maxdepth 4 -type f | grep -Ei 'auth|user' | sort" >> "%LOG%" 2>&1
echo === find service files === >> "%LOG%"
docker exec ragflow-v0256-upgrade-ragflow-cpu-1 sh -lc "find /ragflow/api/db -maxdepth 4 -type f | grep -Ei 'user|account|tenant' | sort" >> "%LOG%" 2>&1
echo === auth grep selected dirs === >> "%LOG%"
docker exec ragflow-v0256-upgrade-ragflow-cpu-1 sh -lc "grep -R -n -E 'auth/login|login_channels|def login|def signin|check_password|decrypt' /ragflow/api/apps/restful_apis /ragflow/api/apps/sdk /ragflow/api/db/services 2>/dev/null | head -200" >> "%LOG%" 2>&1
