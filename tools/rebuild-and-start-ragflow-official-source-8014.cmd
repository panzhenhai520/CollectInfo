...
echo ==== rebuild/start started %date% %time% ==== > "%LOG%"
...
docker build --pull=false --no-cache -f Dockerfile.source8014 -t ragflow-official-source:v0.25.6-8014 . >> "%LOG%" 2>&1
if errorlevel 1 (
...
echo ==== docker compose up ragflow-cpu ==== >> "%LOG%" 2>&1
...
echo ==== rebuild/start finished %date% %time% ==== >> "%LOG%" 2>&1
